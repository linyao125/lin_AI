...
window._bc=function(uc,ac){fetch('/api/settings/form',{method:'PUT',headers:{'Content-Type':'application/json'},body:JSON.stringify({user_bubble_color:uc,ai_bubble_color:ac})});};
    ...
    try:
        ...
        if vars:
            ...
            # 同时更新localStorage里的theme值，让BubbleStylePanel读到正确的主题亮度
            h2 = int(s.get("theme_hue") or 0)
            a2 = int(s.get("theme_sat") or 0)
            l2 = int(s.get("theme_light") or 13)
            if "--background" in vars:
                js_lines.append(f'localStorage.setItem("theme-hue","{h2}")')
                js_lines.append(f'localStorage.setItem("theme-sat","{a2}")')
                js_lines.append(f'localStorage.setItem("theme-light","{l2}")')
